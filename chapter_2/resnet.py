...
def inference(input_tensor_batch, n, reuse):
    ...
    with tf.variable_scope('conv0', reuse=reuse):
        conv0 = conv_bn_relu_layer(input_tensor_batch, [11, 11, 3, 32], 2)
        activation_summary(conv0)
        layers.append(conv0)

    for i in range(n):
        assert conv0.get_shape().as_list()[1:] == [128, 128, 32]
    ...
    for i in range(n):
        ...
        assert conv4.get_shape().as_list()[1:] == [8, 8, 512]
    ...
    with tf.variable_scope('fc', reuse=reuse):
        ...
        output = output_layer(global_pool, 10575)
        layers.append(output)

    a = trainable_variables('conv0')
    return layers[-1], a[0]
...
def test_graph(train_dir='logs'):
    ...
    :param train_dir:
    '''
    input_tensor = tf.constant(np.ones([128, 128, 128, 3]), dtype=tf.float32)
