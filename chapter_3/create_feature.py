...
if __name__ == '__main__':
    ...
    output, filt1, filt2, last_feature = inference(x,train_flag,keep_prob)
    ...
    with tf.Session(config=config) as sess:
        ...
        if train_switch:
            ...
            start = time.time()
            print('Start Manipulation DATA:')

            index = -1
            # 1000 * 128 + 100 - 1

            for it in range(3641):
                batch_x, batch_y = sess.run([train_batch_image, train_batch_labels])
                feature = sess.run(last_feature,feed_dict={x:batch_x, keep_prob: 1.0, train_flag: False})

                for ii in range(batch_size):
                    index += 1

                    feature_data.append(feature[ii])

                    if (index % 89 == 0):
                        print("loading : %.3f" %(index/total) , end='\r')
            ...
            np.save('feature.npy', feature_data)
            start = time.time() - start
            print("Total Time = %f, operations is completed" % start)        
            ...
            # visualize_filters(sess=sess, layer=filt1)
